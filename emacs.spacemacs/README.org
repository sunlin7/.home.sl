...
#+begin_src emacs-lisp :tangle "dot-emacs.el"
  ...
  (let* ((init-file (expand-file-name ".emacs" portable-root-dir))
         (init-template
          (expand-file-name
           ".home.sl/emacs.spacemacs/dot-emacs-template.el" portable-home-dir)))
    (when (and (not (fboundp 'image-mask-p)) (file-exists-p init-file))
      ;; for noX build, try ~/.rootm-*/.emacs first
      (setq portable-home-dir portable-root-dir))
    (when (file-exists-p init-template)
      (setq user-init-file init-template)
      (load (file-name-sans-extension init-template))))
#+end_src
** Addtional Requirest
...
** Shortcuts from Windows to Remote
   ...
   *shortcut for mingw64*
   C:\msys64\mingw64.exe bash -lc "runemacs --daemon"
   C:\msys64\mingw64\bin\emacsclientw.exe -c
