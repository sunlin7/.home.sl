...
#+begin_src emacs-lisp :tangle "dot-emacs.el"
  ...
  (when (equal system-type 'windows-nt)   ; more executables from cygwin, should before other path
    ...
    (with-eval-after-load 'org ; org-mode on windows will try call associated application
  ...
  (defvar portable-home-dir
    (if (and (null (fboundp 'image-mask-p)) ; try .root[im]-*/.emacs for noX build
        ...
        portable-root-dir
      (expand-file-name ".." (file-truename (or load-file-name (buffer-file-name))))))

  (defvar portable-running-now (equal portable-root-dir portable-home-dir))
  ...
  (custom-set-variables
   '(window-system-default-frame-alist
     '((x . ((left . (+ 16)) (top . (+ 72)) (width . 190) (height . 50)))
       (w32 . ((left . (+ -9)) (top . (+ -5)) (width . 190) (height . 52)))))
   ...
   '(native-comp-jit-compilation (not portable-running-now))
   '(native-comp-enable-subr-trampolines (not portable-running-now))
